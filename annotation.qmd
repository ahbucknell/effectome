...
### Setup
Currently I'm visualising the tree using ITOL, and therefore am saving annotation datasets as `.txt` files to upload. ITOL annotation files use a custom header depending on the annotation type. Below outlines header generation function depending on the annotation type (binary or colour-scale).
```{r}
library(ape)
...
### Tree nodes
The below section reads the Newick tree as input and generates a map dataframe between MGG and the unique prefix ID appended during AlphaFold2 structural prediction.
```{r}
...
### MAX effectors
This section just identifies known and predicted MAX effectors within the dataset used in the phylogeny.
```{r}
#max_mgg contains duplicates, but max_mgg_tree only returns unique values intersecting.
max_mgg <- read.csv(file.path(dir, "MAX_MGG.csv"), header = FALSE)$V1
max_mgg_tree <- intersect(max_mgg, tree_map$mgg)
max_nodes <- tree_map$node[tree_map$mgg %in% max_mgg_tree]
max_colour <- rgb(194, 106, 119, maxColorValue = 255)
out_max_path <- file.path(dir, "MAX.itol.txt")
writeLines(binary_header("MAX", max_colour), out_max_path)
write(paste(max_nodes, 1, sep = ","), file = out_max_path, append = TRUE)
```
